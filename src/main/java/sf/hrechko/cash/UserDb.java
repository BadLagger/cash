...
public class UserDb extends FileDb {
	...
	CashCategory(String name, int value) {
	    this.name = name;
	    this.value = value;
	}
	...
	public boolean setIncome(CashCategory cash) {
		...
		int currentValue = foundCategory.getValue() + cash.getValue();
	...
	try {
		...
		while (revCategories.hasNext()) {
		    ...
		    int revValue = revenue.get(revName).asInt();
		...
		while (spCategories.hasNext()) {
		    ...
		    int spValue = spending.get(spName).asInt();
